...
/*
 state 5: like stage 4 but the seam runs horizontally
 */

fun transposeImage(orig: BufferedImage): BufferedImage {
    val transposedImage = BufferedImage(orig.height, orig.width, BufferedImage.TYPE_INT_RGB)
    (0 until orig.height).forEach { y ->
        (0 until orig.width).forEach { x ->
            transposedImage.setRGB(y, x, orig.getRGB(x, y))
        }
    }
...
fun main(args: Array<String>) {
    ...
    val transposed = transposeImage(myImage)
    Seam.originalImage = transposeImage(myImage)
    val energies = transposed.applySourceIntensity()
    val matrix: MutableList<Double> = mutableListOf()
    (0 until transposed.width).forEach {
        matrix.add(0.0)
    }
    ...
    ImageIO.write(transposeImage(Seam.originalImage), "png", outputFile)
}